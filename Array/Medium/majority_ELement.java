/*if in question says element exists then use one pass Boyer-Moore Voting Algorithm (optimal)
    if it doesn't says that element exists or says may or may not exists then use two pass boyer moore voting tc ans sc is O(n) andO(1)
     Boyer-Moore Voting Algorithm finds the majority element by repeatedly canceling out different elements. The majority element appears more than n/2 times, so it cannot be completely canceled and survives as the final candidate.
